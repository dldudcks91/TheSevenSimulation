...
- 기획서에서 수치를 발견하면 그 값이 **CSV에 존재하는지 먼저 확인**.
  - 기획서 수치 ≠ CSV 값이면 **CSV가 진실** (기획서가 오래된 것).
  - 발견 시 사용자에게 보고하고 `docs/_migration_findings.md` 추가/검토 + 기획서 정정 제안
  - CLAUDE.md "기획 문서에도 절대 수치 기재 금지" 정책 준수
