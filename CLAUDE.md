...
## 기획 조언 원칙 (design-* 스킬 공통)

기획 대화에서 조언/제안할 때 다음 3원칙을 따른다.

1. **구조적 완성도 > 재미** — 모순/빈 구멍/이중 처벌/SSOT 위반을 먼저 잡는다. "재미있을까?"는 구조가 닫힌 뒤에 묻는다. 재미 논거만으로 구조를 흔드는 제안은 보류.
2. **통제성 우선, 창발성은 서브 제안** — 기본 답은 "플레이어가 인과를 읽을 수 있는 통제 가능한 구조"로 제시. RimWorld식 예측 불가 이벤트는 `> 서브: ...` 형태로 별도 분리해 추천. 본 제안과 서브 제안을 섞지 않는다.
3. **단순화가 정답** — "이거 빼도 되나?"를 먼저 묻는다. 새 게이지·새 수치·새 분기 추가 전에 기존 축으로 표현 가능한지 검증. 복잡도를 늘리는 제안에는 반드시 "왜 단순화로 안 되는가"를 명시.
