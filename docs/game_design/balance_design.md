...
## 3. 사기(Morale) 밸런스 (메인 죄종 + 사기 시스템 — 2026-04-27 롤백)

> **2026-04-27**: 7수치 누적 프레임 폐기. **사기 1~100 단일 게이지** + modifier 합산식이 영웅 관리 레버.
> 영웅 정체성은 **메인 죄종 1개 영웅별 고정** (이름과 1:1).
> 본 섹션은 뼈대 — 구체 수치(modifier offset/TTL, 폭주 확률 곡선)는 별도 세션에서 확정.

### 3-1. 문제 정의

사기 시스템이 영웅 운영의 기준. 밸런스가 맞지 않으면:
- **너무 빠르게 회복** → 폭주/드라마가 거의 없음 → 거점 운영이 단조로움
- **너무 빠르게 하락** → 매 턴 여러 영웅이 폭주 → 관리 불가 → 좌절
- **modifier 너무 약함** → 사건이 영웅에게 의미 있는 흔적을 남기지 않음
- **modifier 너무 강함** → 한 사건이 사기 임계까지 즉시 끌어내림 → 회복 불가

### 3-2. 설계 방향

**"초반엔 안정, 중반에 흔들림, 후반엔 선택 딜레마"**

목표:
- 영웅 초기값 = 사기 50 (정상)
- 챕터 1 초반(Day 1~5): 사기 40~60 안정
- 챕터 1 중반(Day 8~12): memory modifier 누적으로 일부 영웅 30 근접 (불안 영역)
- 챕터 1 후반(Day 15~20): 첫 폭주 위험 체감
- 7명 중 **2~3명은 항상 "사기 관리 필요"** 영웅 (관리 우선순위 형성)
- 폭주/구원은 희소 이벤트 (30턴에 폭주 2~4회 + 구원 1~2회)

### 3-3. 사기 modifier 카탈로그 (수치 TBD)

#### Memory modifier (사건 후 시간 만료) 후보

| Source | offset | TTL (턴) | 비고 |
|---|---|---|---|
| 전투 승리 | +5 | 3 | |
| 전투 패배 | -5 | 5 | |
| 동료 사망 | -25 | 20 | 다단계 감쇠 |
...
| 불리한 판결 | -8 | 5 | |
| 연회 참석 | +6 | 3 | |
| Catharsis (폭주 직후) | +30 | 1 | 안전장치, 자동 |
| 구원 시도 성공 | +20 | 5 | |
| 같은 영웅 폭주 목격 | -3 | 3 | 연쇄 |

#### Situational modifier (조건 활성/비활성)

| Source | offset | 조건 |
|---|---|---|
| 침대 등급 | +0 ~ +5 | 숙소 시설 |
| 1인실 | +3 | 숙소 정원 여유 |
| 식사 등급 | -3 ~ +5 | 매 턴 식량 풍족도 |
| 전장 배치 | -3 | 원정/방어 진행 중 |
| 좋아하는 작업 | +5 | 시작특성 매칭 시 |
| 부상 상태 | -5 | injured 동안 |

#### Permanent modifier (시작특성 영구)

| 특성 | offset | 비고 |
|---|---|---|
| 낙천적 (유나) | +12 | RimWorld Sanguine |
| 둔감한 (오린) | 0 (변동폭 -30%) | 사기 변동 0.7배 |
| 예민한 (라엘) | 0 (변동폭 +30%) | 사기 변동 1.3배 |
| 철의 의지 (가론) | 0 (폭주 임계 -5) | 임계 변경 |

> 모든 수치는 `balance.csv` `morale_mod_*` 키로 이전 예정.
...
morale_max,100
morale_initial,50
morale_break_threshold,30          # 이하 = 매 턴 폭주 확률 체크
morale_low_threshold,40            # 이하 = 불안 영역 (능력 -)
morale_high_threshold,70           # 이상 = 고양 영역 (능력 +)
morale_inspiration_threshold,80    # 이상 = 매 턴 구원 확률 체크
morale_break_chance_curve_*        # 폭주 확률 곡선
morale_inspiration_chance_curve_*  # 구원 확률 곡선
morale_catharsis_offset,30         # 폭주 직후 +30
morale_catharsis_ttl,1             # 1턴 지속
```

### 3-5. 폭주/구원 빈도 시뮬레이션 (재설정 필요)

7명 영웅, 30턴 플레이에서 예상 이벤트 수:

| 이벤트 | 예상 빈도 | 30턴 중 예상 횟수 |
|---|---|---|
| 사기 고양 진입(70+) | 영웅당 2~4회 | 총 14~28회 |
| 사기 불안 진입(40-) | 영웅당 2~3회 | 총 14~21회 |
| 폭주 (사기 30 이하 + 확률) | 영웅당 0~1회 | 총 2~4회 |
| 구원 (사기 80 이상 + 확률) | 영웅당 0~1회 | 총 1~2회 |
| 연쇄 반응 (폭주 → 사기 modifier 전파) | 폭주 시 50% | 총 1~2회 |

### 3-6. 연쇄 반응 — 사기 modifier 전파

폭주 결과가 다른 영웅에게 사기 modifier를 추가해 추가 폭주를 유발할 수 있음.

| 폭주 결과 | 영향 영웅 | modifier (수치 TBD) |
|---|---|---|
| 분노 폭주 (도발) | 대상 영웅 | "도발당함" -10 (memory 5턴) |
| 탐욕 폭주 (횡령) | 전체 | "굶주림" -3 (situational, 식량 부족 동안) |
| 교만 폭주 (명령 거부) | 사기 낮은 영웅 | "지휘 붕괴" -5 (memory 3턴) |
| 시기 폭주 (사보타주) | 강한 영웅 | "방해받음" -8 (memory 5턴) |
| 폭식 폭주 (식량 소진) | 전체 | "굶주림" -3 (situational) |
| 나태 폭주 (전파) | 주변 영웅 | "무기력 전염" -5 (memory 3턴) |
| 색욕 폭주 (분쟁) | 관련 2명 | "분쟁 휘말림" -5 (memory 3턴) |

**연쇄 깊이**: 최대 3회 (`max_chain_reactions`) — 유지.
**Catharsis 안전장치**: 폭주 직후 +30 사기 1턴 자동 부여 → 무한 루프 방지.

### 3-7. 사기 밸런스 체크리스트

| 체크 | 질문 | 목표 |
|---|---|---|
| □ | Day 1~5에 영웅 사기가 대부분 40~60 구간인가? | 안정 초반 |
| □ | Day 8~12에 사기 30 근접 영웅이 등장하는가? | O (위기감) |
| □ | Day 15~20에 첫 폭주가 체감되는가? | O |
| □ | 사기 회복 수단(연회/숙소/판결)이 "쓰고 싶지만 아까운" 긴장감을 주는가? | O (자원·기회비용) |
| □ | 7명 중 2~3명은 항상 사기 관리가 필요한가? | O |
| □ | 연쇄 반응이 "옆 영웅에게 번지는구나"를 느끼게 하는가? | O |
| □ | 폭주가 너무 자주 일어나 짜증나지는 않는가? | 30턴에 2~4회 (희소) |
| □ | Catharsis가 무한 폭주를 방지하는가? | O |
| □ | 사기 80+ 고양 영역이 의미 있는 보상(구원)을 주는가? | O |
| □ | UI에 "왜 이 사기가 이 값인지" 분해해서 보이는가? | O (modifier 표시) |
...
| 순위 | 영역 | 구체 항목 | 현재 상태 |
|---|---|---|---|
| **1** | 사기 시스템 | modifier offset/TTL, 폭주/구원 확률 곡선, 임계값 | 메인+사기 롤백 직후, 수치 TBD |
| **2** | 전투 | 방어전 스케일링 곡선 | 현재 너무 가파름 |
| **3** | 경제 | 식량 수지, 건설 비용 | 식량 고갈 너무 빠름 |
| **4** | 전투 | 사냥 보상 밸런스 | 현재 적절 |
| **5** | 이벤트 | 선택지 사기 modifier (offset/TTL) | 초안 있음 (재태깅 필요) |
| **6** | 경제 | 연구 비용/효과 | 현재 적절 |
...
- **현재 등록**: 분노(전체 사기), 탐욕(골드 -80), 색욕(랜덤 1명 사기) — 사기 시스템 롤백으로 사기 기준 재정의 (offset/TTL 명시)
- **none 처리**: 시기/나태/폭식/교만 — 추가 효과 미정. 7죄종 중 4건이 빈 행

> **재설계 후보 (별도 작업)**: 사기 modifier 기반으로 일괄 정의 + none 4건 채우기. 시기→건물 다운 + 사기 modifier, 폭식→식량 차감 + 사기, 교만→전 영웅 사기 -10, 나태→무효과(서사상 "조용한 이탈"). 적용은 `desertion_effects.csv` 컬럼 재설계와 함께 — `_migration_findings.md` 신규 finding 등록.
...
*마지막 업데이트: 2026-04-27 (메인 죄종 + 사기 1~100 시스템 롤백 — §3 사기 밸런스 뼈대 재작성, modifier 카탈로그 도입, 7수치 누적 관련 조항 모두 폐기)*

*이전: 2026-04-27 Phase B — CSV SSOT 분리 / 2026-04-20 사기→죄종 수치 전환 / 2026-04-17 쌓임 프레임 재정립 (모두 본 롤백으로 사기 시스템 환원)*
...
- **사기 모드 환원** (2026-04-27): `exp_node_rest_*`, `exp_node_victory_*`, `exp_node_defeat_*` — 사기 시스템으로 환원. 사기 modifier 부여 (offset/TTL) 형태로 재정의. 코드 사용처 검증 후 명명 통일 (`*_morale_offset` / `*_morale_ttl`).
...
- **재활성 키**: `sickness_morale_delta` — 사기 시스템 롤백으로 다시 유효. 발병 시 사기 -N modifier 부여 (memory, TTL=발병 지속 턴).
