...
> 상태: 초안 — 3개 영역 예시 + 방향성
> 작성일: 2026-03-29
> 최종 수정: 2026-04-15 (영웅 HP 제거, 스태미너 기반 전투 수치로 전환)
...
현재 전투 관련 수치:
- 영웅: **HP 없음** — 병사(HP=80)가 내구력 담당. 참전 시 스태미너 -30 소모
- 적 (챕터1): HP 40~300, ATK 8~30
- 방어전 적: HP 120+day×8, ATK 12+day×2 (→ 1-5 로그 곡선 개정안 참조)
- 사냥 적: HP 25~70, ATK 8~14

**문제**: 영웅이 성장하지 않으면, 적은 일수에 비례해 강해짐 → 언제 역전되는가? 그 곡선이 재밌는가?
...
> 공식: ATK(원정) = 힘×0.7 + 민첩×0.4, ATK(방어) = 힘×0.8 + 통솔×0.4
> 영웅 HP 없음 — 내구력은 병사 수로 결정 (병사 HP=80)
> 건강 = 스태미너(체력) 최대치/회복 + 발병 저항 전용
...
**문제**: day 20이면 적 HP=280, ATK=52 → 영웅 평균 HP 350을 2타에 잡음. 너무 가파름.
...
```
영웅 ATK 11 (평균) vs 들늑대 HP 40 → 약 4턴 킬 (스태미너 여유 시 안전)
들늑대 ATK 8 → 병사 없는 사냥에서 영웅 직접 피격 → 스태미너 소모로 판정

영웅 ATK 5 (허약형) vs 변이 곰 HP 70 → 약 14턴 킬
→ 전투 장기화 → 스태미너 추가 소모 → 허약형은 사냥 후 부상 위험 존재
```
> 사냥은 병사 없이 1:1 전투 — 적의 피격이 스태미너에 직접 영향. 상세 수치 미확정.

### 1-5. balance.csv 반영 제안

```csv
# 전투 스태미너 비용
battle_stamina_cost,30
hunt_stamina_cost,30

# 부상 판정 임계 (병사 전멸 시 남은 스태미너 기준)
injury_threshold_safe,30
injury_tier1_max,30
injury_tier2_max,20
injury_tier3_max,10
# tier1(21~30): 경상70 중상30 사망0
# tier2(11~20): 경상50 중상40 사망10
# tier3(1~10):  경상25 중상50 사망25
# tier4(0):     경상10 중상40 사망50
...
|---|---|---|---|
| BattleEngine | SP 최대 100 | line 62 | sp_max |
| BattleEngine | SP/틱 +2 | line 63 | sp_per_tick |
| BattleEngine | SP/킬 +10 | line 64 | sp_per_kill |
