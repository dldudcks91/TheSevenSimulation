...
## 3. 사기/죄종 밸런스 — 변동 빈도 설계

### 3-1. 문제 정의

사기 시스템은 영웅 운영의 기준 게이지. 밸런스가 맞지 않으면:
- **너무 안정적** → 영웅의 만족/불만 차이가 의사결정을 만들지 못함 → 거점 운영이 단조로움
- **너무 불안정** → 매 턴 사기가 양극단으로 튐 → 관리 불가 → 좌절
- **편차 없음** → 모든 영웅이 비슷한 사기 → 영웅별 차별화 약화

### 3-2. 설계 방향

**"평균은 안정, 가장자리는 살아 움직인다"**

목표:
- 평균 사기 = 50 (안정 구간 중앙)
- 5~8턴마다 1명은 위험 구간(0~25 또는 75~100)에 진입
- 폭주/이탈은 드물게(10~15턴 1회 정도) 발생, 평소엔 만족/불만 변동이 주요 압박
- 7명 중 **2~3명은 항상 사기가 흔들림** (관리 우선순위가 생기는 정도)
...
| 체크 | 질문 | 목표 |
|---|---|---|
| □ | Day 5~8에 영웅 간 사기 편차가 명확히 갈리는가? | O |
| □ | "이 영웅의 만족/불만이 운영 결정을 바꾼다" 압박이 있는가? | O |
...
| `exp_node_rest_morale` | 10 | 야영 노드 시 파티 사기 증가량 |
| `exp_node_victory_morale` | 3 | 전투 노드 승리 시 파티 사기 증가량 |
| `exp_node_defeat_morale` | -8 | 전투 노드 패배 시 파티 사기 변동 |
...
| `sickness_morale_delta` | -8 | 발병 시 사기 변동 |
